...
public class User {
    ...
    public void setUnreedMessagesToZero(String mailOther) {
    ...
    public void addMessage(String mail2, String message2) {
        for (int i = 0; i < chats.size(); i++) {
            if (chats.get(i).getMail().equals(mail2)) {
                chats.get(i).setUnreadMessage(chats.get(i).getUnreadMessages() + 1);
                chats.get(i).setLastMessage(message2);
                chats.get(i).setTime(new Date().getTime());
            }
        }
    }

    public void addMyMessage(String mail2, String message2) {
        for (int i = 0; i < chats.size(); i++) {
            if (chats.get(i).getMail().equals(mail2)) {
                chats.get(i).setLastMessage(message2);
                chats.get(i).setTime(new Date().getTime());
